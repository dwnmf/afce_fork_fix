...
void QBlock::adjustSize(const double aZoom)
{
  ...
  else
  {
    ...
    else if (type() == "process")
    {
      ...
      double textWidth = fm.horizontalAdvance(text) + 16 * aZoom;  // доп. отступы
    ...
    else if (type() == "io")
    {
      ...
      // Вычисляем ширину текста переменных
      QFont font("Tahoma");
      font.setPixelSize(13 * aZoom);
      QFontMetrics fm(font);
      QStringList vars = attributes.value("vars", "").split(",");
      QString text = vars.join(", ");
      double textWidth = fm.horizontalAdvance(text) + 20 * aZoom;  // доп. отступы
      double minWidth = 120 * aZoom;
      clientWidth = qMax(minWidth, textWidth);
      clientHeight = 60 * aZoom;
    }
